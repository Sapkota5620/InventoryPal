...
def analyze_boxes(matblobs, display_frame, size_threshold=10000):
    ...
    if valid_boxes:
        ...
        for i, b_rect in enumerate(valid_boxes):
            if i < 4:
                ...
                cv2.imwrite(f"cropped{i + 1}.jpg", roi)
...
def main_process():
    ...
    cv2.imshow("canny0", canny_o)
    cv2.waitKey(0)
    thresh_o = cv2.adaptiveThreshold(canny_o, 255, 1, 1, 3, 2)


    # canny = cv2.Canny(blurred,40,180) #images/5_ld_24.jpg -- 13
    canny = cv2.Canny(gray, 1, 200)
    ...
    valid_boxes = analyze_boxes(thresh_o, display_color, size_threshold=threshold*mul) 
    boxesCounter(display_color, valid_boxes)
    cv2.imshow("thresh_0", display_color)
    cv2.waitKey(0)

    display_color = cv2.cvtColor(gray,cv2.COLOR_GRAY2BGR)
    valid_boxes = analyze_boxes(thresh, display_color, size_threshold=threshold*mul) 
    boxesCounter(display_color, valid_boxes)
    cv2.imshow("thresh_display", display_color)
    cv2.waitKey(0)

    display_color = cv2.cvtColor(gray,cv2.COLOR_GRAY2BGR)
    valid_boxes = analyze_boxes(matmorph, display_color, size_threshold=threshold*mul) 
    boxesCounter(display_color, valid_boxes)
    cv2.imshow("matmorph_display", display_color)
    cv2.waitKey(0)
    ...
    plt.show()
    """
    #cv2.imshow("display_colro", display_color)
    cv2.waitKey(300)
